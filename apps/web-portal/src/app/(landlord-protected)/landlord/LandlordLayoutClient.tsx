...
const landlordNav = [
  ...
  { href: "/landlord/tickets", label: "Tickets" },
];

export function LandlordLayoutClient({
  children,
  ...
  const handleSignOut = useCallback(async () => {
    await signOut();
    window.location.assign("/landlord/sign-in");
  }, [signOut]);
    ...
    return (
      <div className="min-h-screen flex flex-col bg-zinc-100 p-6">
        ...
        <Card className="p-6 max-w-md">
          ...
          <button
            type="button"
            onClick={() => signOut().then(() => router.replace("/landlord/sign-in"))}
    ...
    <div className="min-h-screen flex flex-col bg-zinc-100">
      <header className="h-14 border-b border-zinc-200 bg-white flex items-center justify-between px-6">
        <nav className="flex items-center gap-4">
          <span className="text-sm font-medium text-zinc-900">Landlord Portal</span>
